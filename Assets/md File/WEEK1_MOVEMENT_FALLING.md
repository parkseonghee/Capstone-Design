...
### 2-1. 턴을 쓰는 행동은 방향 입력만이 아니다

한 턴을 소비하는 행동이 셋으로 늘었다. 셋 다 위 페이즈를 **똑같이** 통과한다.

| 행동 | 들어오는 곳 | 플레이어 이동 |
|---|---|---|
| 이동·공격·획득 | 방향 입력(버튼·스와이프·키보드) | 이동만 움직인다 |
| **폭탄 설치** | 폭탄 버튼 → `GameLoop.PlaceBomb` | 안 움직인다 |
| **제자리 대기 (빨리 내리기)** | 대기 버튼 → `GameLoop.Wait` | 안 움직인다 |

**빨리 내리기 버튼**: 플레이어가 가만히 있어도 블록이 내려오게 하는 조작이다.
한 번 탭하면 한 턴, 계속 누르고 있으면 일정 간격으로 턴이 연달아 넘어간다.

> **공짜 행동이 아니다.** 대기도 한 턴을 그대로 쓰므로 중력뿐 아니라 **스폰과 폭탄 도화선도 같이 돈다.**
> 오래 누르면 블록이 내려오는 만큼 새 블록도 쏟아진다 — 그게 이 조작의 비용이다.
> 중력만 공짜로 돌려 주면 스폰 압박 없이 판을 정리할 수 있게 돼 밸런스가 무너진다.
>
> 웨이브 진행도는 **적 처치로만** 차므로(밸런싱 v1 §2-1-4) 대기로는 스테이지를 깰 수 없다.

| 만질 것 | 어디서 |
|---|---|
| 첫 턴 뒤 연속 낙하가 시작되기까지의 뜸 | 씬의 `Btn_FastDrop` → `firstRepeatDelay` (현재 0.25초) |
| 누르고 있는 동안의 턴 간격 | 같은 곳 → `repeatInterval` (현재 0.08초) |
| 한 번 누를 때 최대 턴 수 | 같은 곳 → `maxTurnsPerHold` (현재 0 = 제한 없음) |
